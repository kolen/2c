	...
	if(bWasEdit)
	{
		...
		for(int nCol=0;nCol<pGrid->GetColumnCount();nCol++)
		{
			...
			if (pCell)
			{
				...
				Val.SetAt("Ширина колонки",pGrid->m_arColWidths[nCol]);
			}
		...
		if(Selection.Count()>0)
		{
			...
			if(nCol>=0)
			{
				...
				if (pCell)
				{
					...
					p->AddProperty(csPropertyName,"Ширина колонки",pGrid->m_arColWidths[nCol],0,1000);
					bWasEdit=1;
					return;
				}
			}
...
void CDynControl::PrepareDataArray()
{
	...
	if(m_nControlType == ID_DYNBUTTONGRID)//GRID
	{
		...
		AddBoolProperty("Выделение строки",1);
...
void CDynControl::CreateMicroForm()
{
	...
	extern CValue afxFormParam;//параметр для формы
	extern CString afxFormPath;//путь внешней формы
	extern CString afxFormID;//уникальный ид открываемой формы

	afxFormParam.CreateObject("Структура");//String("Микроформа");
	afxFormParam.SetAt("Микроформа",m_Property);
	afxFormPath="";
	afxFormID=""; 

	try
	{
		AfxGetModuleManager()->FindForm(m_csObjName,csFileName,afxMetaObj);
	}
	catch(...)
	{
		//DoError();
	...
	try
	{
		pControl->Create((LPCTSTR )IDD_FORMUNIT,m_pParentWnd);
	}
	catch(...)
	{
	}
