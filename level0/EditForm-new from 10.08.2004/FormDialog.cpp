...
BOOL IsEditGrid(CWnd* pWnd,CPoint point,UINT nMsg)
{
	...
	if(pWnd->IsKindOf(RUNTIME_CLASS(CGridCtrl)))
	{
		...
		if(cell.row==0)
		{
			...
			{
				pThis->SelectItem(pWnd,TRACK_NEW);
...
//	if(!nMode)
	...
	m_pTracker->SetTracker(pWnd,Rect,-1,nMode);
	...
	if(bSelectItem||IsCTRLpressed()||IsSHIFTpressed())//режим выделения элементов
	{
		...
		if(pWnd)
		{
			...
			if(bWasAddControl)
				...
				nMode=TRACK_REMOVE_ALL | TRACK_ADD | TRACK_CHECK_KEY | TRACK_CURRENT;

			SelectItem(pWnd,nMode);
			ShowProperty(0);
			return;
		}
	}
...
void CFormDialog::ShowProperty(int nShow) 
{
	...
	if(pControl)
	{
		...
		AfxGetProperty()->SetDoc(GetDocument(),(LPARAM)pControl,(LPARAM)this);
...
void CFormDialog::PostUpdate()
{
	...
	if(pControl)
	{
		...
		if(pPrevWnd!=pControl->m_pWnd)//объект поменял окно
		{
			...
			if(m_pTracker)
			{
				SelectItem(pControl->m_pWnd,TRACK_NEW);
			}
			//ShowProperty(0);
			ReloadWindows();
		}
...
{ 
	...
	for(int i=0;i<ValCopyPaste.GetSizeArray();i++)
	{
		...
		m_pTracker->SetTracker(pWnd,Rect,-1,TRACK_ADD | TRACK_CURRENT);
