	...
	if(bSelect)
	{
		...
		if(bRes)
		{
			for(int i=0;i<m_pDialog->aWindowsList.GetSize();i++)
			{
				...
				{
					m_pDialog->SelectItem(CWnd::FromHandle(m_pDialog->aWindowsList[i]),TRACK_ADD | TRACK_CURRENT | TRACK_CHECK_KEY);
				}
			}
		}
	}
...
void CFormEditor::OnSelectAll() 
{
	...
	for(int i=0;i<m_pDialog->aWindowsList.GetSize();i++)
	{
		m_pDialog->SelectItem(CWnd::FromHandle(m_pDialog->aWindowsList[i]),TRACK_ADD | TRACK_CURRENT);
	}
...
void CFormEditor::ShowProperty(bool nShow) 
{
	...
	if(m_pDialog)
	{
		AfxGetProperty()->SetDoc(GetDocument(),(LPARAM)m_pDialog,(LPARAM)this);
