...
void CMultiRectTracker::Draw(CDC* pDC)
{
	...
	for(int i=0;i<aList.GetSize();i++)
	{
		...
		if(aWindows[i]==pCurWnd)
			bResize=1;
		Draw(pDC,aList[i],bResize);
	}
}
void CMultiRectTracker::Draw(CDC* pDC,CRect &rect,bool bResize)
{
	VERIFY(pDC->SaveDC() != 0);
...
*/		{
	...
	if (m_bFinalErase || !m_bErase)
	{
		...
		if(TestRect.Height()==m_rect.Height()&&TestRect.Width()==m_rect.Width())//перемещение
		{
			for(int i=0;i<aList.GetSize();i++)
			{
				CPoint delta;
...
void CMultiRectTracker::SetTracker(CWnd *pWnd,LPCRECT lpSrcRect, int nStyle,int nMode)
{
	if(nStyle==-1)
		nStyle=dottedLine  + resizeOutside + resizeInside + hatchedBorder;
	m_nStyle = nStyle;

	//поиск подобного элемента в списке...
	int nFind=-1;
	for(int i=0;i<aList.GetSize();i++)
	if(pWnd==aWindows[i])
	{
...
*/
	...
	if(nMode & TRACK_CHECK_KEY)
	if(IsCTRLpressed()||IsSHIFTpressed())
	{
		if(nFind>=0)
			nMode=TRACK_REMOVE | TRACK_CURRENT;
		else
			nMode=TRACK_ADD | TRACK_CURRENT;
	}

	//очистка всего списка
	if(nMode & TRACK_REMOVE_ALL)
	if(nFind==-1)
	{
		aList.RemoveAll();
		aWindows.RemoveAll();
	...
	//добавление переданного элемента
	if(nMode & TRACK_ADD)
	{
		aList.Add(m_rect);
		nFind=aWindows.Add(pWnd);
	}

	//удаление переданного элемента
	if(nMode & TRACK_REMOVE)
	if(nFind>=0)
	{
		aList.RemoveAt(nFind);
		aWindows.RemoveAt(nFind);
		nFind=aWindows.GetSize()-1;
	}
	
	//установка фокуса текущего элемента
	//if(nMode & TRACK_CURRENT)
	if(nFind>=0)
	{
		m_rect.CopyRect(aList[nFind]);
		pCurWnd=aWindows[nFind];

		//удаляем - т.к. это окно будет текущим...
		//aList.RemoveAt(nFind);
		//aWindows.RemoveAt(nFind);
	}
