...
private:
	...
	FORPLUGIN static CMapStringToPtr ObjectsLibTree;
	FORPLUGIN friend CValue CreateObject(CString Str,CValue Val);
	FORPLUGIN friend void RegisterObject(CRuntimeClass* pRuntimeClass,CString Str,class CModuleManager *pLibrary);
...
public:
	...
	FORPLUGIN virtual void Init(CString StrTypeKind,CValue Param,class CModuleManager *pLibrary);//строка как была передана в функцию СоздатьОбъект, например "Документ.РасходнаяНакладная"
...
FORPLUGIN void RegisterObject(CRuntimeClass* pRuntimeClass,CString Str,class CModuleManager *pLibrary=0);
