...
IMPLEMENT_DYNCREATE(COXStaticText,CStatic);   
...
void COXStaticText::EmbossText(CDC* pMemDC, RECT BmpRect)
{
	...
	if(m_dwBkColor!=CLR_NONE)
		oldBkColor = pMemDC->SetBkColor(m_dwBkColor);
...
// --- Effect :		Performs text output to the compatible memory device context.
{
	...
	{
/*		int angleChar = GetCharAngle()/10;
		int angleString = GetStringAngle()/10;

		if (absolute(angleString)>=absolute(angleString))
		{
			sizeCorr.cy = (int)(((double)sizeCorr.cy)*cos(alphaAdvanced)+
				(cos(alphaAdvanced)>0 ? -0.5 : 0.5));
		}
...
/*
	pMemDC->SetTextAlign(TA_LEFT | TA_BOTTOM );
	pMemDC->SetTextColor(m_dwTextColor);
	CRect textRect;
	GetClientRect(&textRect);
	pMemDC->DrawText(m_sTextNarrow, textRect, DT_NOCLIP | DT_LEFT | DT_NOPREFIX | DT_WORDBREAK);
*/
	...
	for ( DWORD dwCount = 0; dwCount < m_dwOffset; dwCount++ )
	{
		...
		pMemDC->DrawText(m_sTextNarrow, &rectOutput,DT_NOPREFIX | DT_WORDBREAK);//DT_SINGLELINE);
		OffsetRect( &rect, 1,1);
	}
	pMemDC->TextOut( (rect.left+rect.right)/2+sizeCorr.cx, 
		(rect.top+rect.bottom)/2+sizeCorr.cy, m_sTextNarrow);
	CRect rectOutput((rect.left+rect.right)/2+sizeCorr.cx,
		...
		(rect.top+rect.bottom)/2+sizeCorr.cy+rect.bottom-rect.top);
	ScreenToClient(rectOutput);
	pMemDC->DrawText(m_sTextNarrow, &rectOutput,0);
